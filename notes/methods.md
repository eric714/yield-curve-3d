...
Those use overlapping 24-month windows, so the p-value is not to be read as
precise. On **18 non-overlapping blocks** the slope is **0.984** (se 0.404) and
the identity restriction again cannot be rejected (p = 0.97). Little power at
n=18, but it confirms the overlap is not manufacturing the result.
...
**In this system the curve predicts policy changes and the macro variables do
not.** Read that as a statement about *this VAR*, not about the world: section
0 explains why it is not a fair contest. The flat version of this sentence —
"the curve Granger-causes Fed policy, and nothing else does" — stood here
through two rounds of review and was wrong to.

Every number in this section is regenerated by
[`analysis/headline.py`](../analysis/headline.py). If a figure here is not in
that script's output, it is a bug.
...
- **The zero lower bound.** 108 of 437 months sat at or below 0.30%. Excluding
  them, the relationship gets *stronger*: chi2 102.4 versus 92.0 at lag 9.
- **Every Fed chair**, with one caveat. Re-run with HAC errors rather than the
  plain F used in an earlier draft: Greenspan chi2 48.0 (p=2.2e-10, k/n=0.12),
  Bernanke 58.0 (p=1.6e-12, k/n=0.23), Powell 17.5 (p=5.6e-04, k/n=0.22).
  **The Yellen subsample is not usable** — 48 months against 22 parameters,
  k/n=0.46 — and is reported here only so its absence is not silent.
...
**Does 2021-22 carry it?** A reviewer suggested the whole table might rest on
one extraordinary cycle. Splitting the sample says no, and sharpens the result
([`analysis/money.py`](../analysis/money.py)):

| Sample | dM2 → dCPI | dCPI → dM2 |
|---|---|---|
| Full, 1990–2026 | chi2 47.5, p = 3.8e-06 | chi2 22.1, p = 0.036 |
| **Excluding 2020 onward** | **chi2 48.1, p = 3.0e-06** | chi2 14.2, **p = 0.29** |
| 2020 onward only | k/n = 0.46 — not usable | k/n = 0.46 — not usable |

The forward direction is unchanged without the COVID episode. The *reverse*
direction is the part that depended on it. So the bidirectionality is a
2020s phenomenon and the money-leads-inflation direction is not.
...
A third round, the sharpest, found the reproducibility failure:

| Criticism | Verdict |
|---|---|
| **The headline Wald statistic appears in the write-up but in no committed script** | **Correct, and the worst of these.** `analysis/headline.py` now regenerates every figure in section 1 |
| `granger.py` ships the plain-F estimator section 7 calls invalid | **Correct.** Labeled superseded; nothing in the note is sourced from it |
| Chair-era tests reintroduced that same plain F | **Correct.** Redone with HAC — and the Yellen subsample turned out to be unusable at k/n = 0.46 |
| `dat.py` and `macro.py` hardcoded an absolute path | **Correct.** They run from the repository root now |
| Newey-West bandwidth differs by file and is never stated | **Correct.** `headline.py` prints it on every table (lag + 4) |
| Section 1 still asserted "nothing else predicts policy" after section 0 conceded it | **Correct.** "Ship one document" was the right phrase |
| The identity test uses overlapping windows | **Correct.** Re-run on 18 non-overlapping blocks: slope 0.984, same conclusion |
| The money result may rest on 2021–22 alone | **Tested, and no.** Excluding 2020 onward the forward direction is unchanged; the reverse direction is what disappears |
| `dCPI`/`dM2` may be differences of the raw index, not of YoY rates | **Checked, and they are YoY.** The stored series ranges −2.0 to 9.0 |
| The 29% is descriptive of this VAR, not a structural share | **Correct**, and now said in section 0 |
